<template>
    ...
    <a-table :dataSource="dailyTrainStations" :columns="columns" :pagination="pagination" @change="handleTableChange"
        :loading="loading">
    ...
    <a-modal v-model:visible="visible" title="每日车站" @ok="handleOk" ok-text="确认" cancel-text="取消">
        <a-form :model="dailyTrainStation" :label-col="{ span: 4 }" :wrapper-col="{ span: 20 }">
            ...
            <a-form-item label="车次编号">
                <train-select-view v-model:value="dailyTrainStation.trainCode" @change="onChangeCode" />
...
watch(
    ...
    () => {
        if (Tool.isNotEmpty(dailyTrainStation.value.name)) {
            ...
            dailyTrainStation.value.namePy = pinyin(dailyTrainStation.value.name, {
                pattern: 'first',
                toneType: 'none',
            }).replaceAll(' ', '');
        } else {
...
const onChangeCode = (dailyTrainStation) => {
    let t = Tool.copy(dailyTrainStation)
    delete t.id
