<template>
  ...
  <a-table :dataSource="dailyTrains" :columns="columns" :pagination="pagination" @change="handleTableChange"
    :loading="loading">
...
watch(
  ...
  () => {
    if (Tool.isNotEmpty(dailyTrain.value.start)) {
      ...
      dailyTrain.value.startPy = pinyin(dailyTrain.value.start, {
        pattern: 'first',
        toneType: 'none',
      }).replaceAll(' ', '');
    } else {
      dailyTrain.value.startPinyin = '';
      dailyTrain.value.startPy = '';
    }
  },
  { immediate: true }
);
...
watch(
  ...
  () => {
    if (Tool.isNotEmpty(dailyTrain.value.end)) {
      ...
      dailyTrain.value.endPy = pinyin(dailyTrain.value.end, {
        pattern: 'first',
        toneType: 'none',
      }).replaceAll(' ', '');
    } else {
      dailyTrain.value.endPinyin = '';
      dailyTrain.value.endPy = '';
    }
  },
  { immediate: true }
);
